...
Fork af study3/simulation.py. Eneste ændring i forhold til Study 3:
    7. predict_nps_study3 accepterer et nps_intercept-argument, så
       NPS_PRED_INTERCEPT kan varieres som eksperimentel faktor.

Når interceptet sænkes (fx fra 10.22 til 8.0), krydser NPS_hat-fordelingen
midtpunktet 7.5. Det gør |NPS_hat − 7.5| V-formet i predicted_throughput
og bryder den monotone rangorden der gør NPS ≡ LRTF i Study 3.

Alle øvrige ændringer fra Study 3 (latent Z, hard/soft mode, paret seeding)
er bibeholdt uændret.
"""
...
def predict_nps_study3(predicted_throughput_minutes: float,
    ...
    """
    Study 3b: forudsig NPS fra predicted throughput med variabelt intercept.

    Study 3b-udvidelse: nps_intercept kan varieres som eksperimentel faktor.
    Når interceptet sænkes (fx til 8.0), krydser NPS_hat-fordelingen
    midtpunktet 7.5, hvilket bryder monotonicitet i |NPS_hat − 7.5|
    og giver NPS-prioritering en anden rangorden end LRTF.

    Args:
        predicted_throughput_minutes: Forudsagt throughput i minutter
        nps_intercept: Intercept for NPS-prædiktionsmodellen (default: 10.2211)
    """
...
def generate_all_arrivals(d_end: int,
    ...
    """
    Study 3b: Generér sager med latent Z og variabelt NPS-intercept.

    Identisk med Study 3, men nps_intercept kan varieres som faktor.
...
def simulate_timeline(discipline: str, n_agents: int, sla_hours: Optional[float],
                       ...
                       nps_intercept: float = NPS_PRED_INTERCEPT) -> SimulationMetrics:
    """
    Kør en komplet Study 3b-simulering.
