...
Fork of study3d/simulation.py. Single mechanical change vs Study 3d:

   13. New helper build_target(f) that constructs an 11-bin multinomial
       with mass f symmetrically split across bins 7 and 8 (mode = 7.5)
       and the remainder distributed uniformly over the other nine bins.
   14. run_single_simulation() accepts a "target_f" key in params; when
       discipline == "NPS_BINNED" and target_f is not None, the target
       multinomial is built from f via build_target(f) and passed to
       apply_rank_binning. If target_f is None, falls back to the
       empirical multinomial (Study 3d behaviour).

The simulation, queue-management, paired-seeding, and rank-binning logic
are unchanged from Study 3d. Only the *target* multinomial varies.
"""
...
# =============================================================================
# STUDY 3-SPECIFIC CONSTANTS + EMPIRICAL MULTINOMIAL
# =============================================================================
...
# =============================================================================
# STUDY 3e — TARGET MULTINOMIAL FAMILY
# =============================================================================
...
def build_target(f: float) -> List[float]:
    ...
    Args:
        f: float in [0, 1]. Total mass at bins 7 and 8.

    Returns:
        List of 11 probabilities summing to 1.
    """
    if not (0.0 <= f <= 1.0):
...
# =============================================================================
# MODEL FUNCTIONS — unchanged from Study 3d
# =============================================================================
...
# =============================================================================
# RANK-BINNING (Study 3d core mechanism, unchanged)
# =============================================================================
...
def apply_rank_binning(cases: List[Case],
    ...
    onto the target multinomial distribution.
...
# =============================================================================
# ALGORITHMS (unchanged from Study 3d)
# =============================================================================
...
def simulate_timeline(discipline: str, n_agents: int, sla_hours: Optional[float],
    ...
    """
    Run one simulation. NPS_BINNED requires either a target_multinomial
    explicitly, or a fallback to EMPIRICAL_NPS_MULTINOMIAL.
    """
    if seeds is None:
    ...
    if discipline == "NPS_BINNED":
        ...
        # Study 3e diagnostics: top priority class size + bulk-class size
...
def derive_seeds(replication: int, rho_idx: int) -> Dict[str, int]:
    """
    Same paired seeding scheme as Study 3/3b/3c/3d. Note: arrivals,
    actual, simulation seeds depend only on replication; pred_noise
    varies with rho_idx. tie_break_key is drawn from rng_arrivals so it
    is invariant in rho and in target_f — same case has same key
    across all (rho, f) cells of a given replication.
    """
...
def run_single_simulation(params: dict) -> dict:
    ...
    # Study 3e: target_f is the new sweep parameter. None → empirical.
    # Only consumed by NPS_BINNED; ignored for other disciplines.
...
if __name__ == "__main__":
    ...
    print()
    print("Sample target multinomials by f:")
    for f in [0.0, 0.145, 0.50, 1.0]:
        p = build_target(f)
        print(f"  f={f:.3f} -> {[round(x, 4) for x in p]} (sum={sum(p):.4f})")
    print()

    f_levels = [0.0, 0.145, 0.50, 1.0]
    for f in f_levels:
        print(f"--- discipline = NPS_BINNED, target_f = {f}, "
              f"6 agents, ρ = 1.0, hard, 60 days, rep = 1 ---")
        ...
        print(f"  top class: {result['top_class_size']} | "
              f"bulk class: {result['bulk_class_size']}")
        print()

    print("Reference disciplines (target_f ignored):")
    for disc in ["FCFS", "LRTF", "SRTF", "NPS"]:
        ...
        print(f"  {disc:>5}: orgNPS = {result['organisation_nps']:+.2f}, "
              f"avgNPS = {result['avg_individual_nps']:.3f}")
