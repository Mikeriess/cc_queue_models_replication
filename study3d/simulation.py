...
Fork of study3c/simulation.py. Changes vs Study 3c:
    9. New field Case.predicted_nps_binned (integer 0..10).
   10. New function apply_rank_binning(cases, target_multinomial) that
       sorts cases by raw predicted_nps ascending and assigns discrete bins
       0..10 using the cumulative empirical multinomial from the case-company
       data (loaded from results/empirical_nps_multinomial.json).
   11. New field Case.tie_break_key (uniform [0,1) drawn from rng_arrivals
       at case creation), used for random tie-breaks within priority classes.
   12. New queue discipline "NPS_BINNED": sort by
       (|predicted_nps_binned − 7.5|, tie_break_key) ascending. Cases binned
       to 7 or 8 get top priority (distance 0.5); the random key resolves
       ties within a class.

The original Study 2/3 NPS discipline ("NPS") is unchanged and uses the
continuous predicted_nps from Eq. 9.

Topic-aware NPS prediction (Study 3c) is dropped — Study 3d isolates the
rank-binning mechanism. nps_intercept stays at the paper baseline (10.22).
"""
...
# =============================================================================
# STUDY 3-SPECIFIC CONSTANTS + EMPIRICAL MULTINOMIAL (Study 3d)
# =============================================================================
...
# =============================================================================
# MODEL FUNCTIONS — unchanged from Study 3c (topic_aware dropped)
# =============================================================================
...
# =============================================================================
# RANK-BINNING (Study 3d core mechanism)
# =============================================================================

def apply_rank_binning(cases: List[Case],
                       target_multinomial: List[float]) -> None:
    """
    Set case.predicted_nps_binned by rank-mapping continuous predicted_nps
    onto the empirical multinomial distribution.

    The transformation is rank-preserving: the case with the lowest raw
    predicted_nps is assigned to bin 0, the next batch to bin 1, etc.,
    with batch sizes given by N * target_multinomial[k]. Any rounding
    leftover is absorbed into the modal bin (10).

    Args:
        cases: List of Case objects (modified in-place).
        target_multinomial: Length-11 list of probabilities for bins 0..10.
    """
    if not cases:
        return
    n = len(cases)

    # Allocate counts per bin via cumulative rounding (no per-rep stochasticity:
    # the same target distribution always produces the same per-bin counts
    # given N).
    cum_target = np.cumsum(target_multinomial)
    cum_alloc = np.round(cum_target * n).astype(int)
    cum_alloc[-1] = n  # force last bin to absorb rounding leftover
    counts = np.diff(np.concatenate([[0], cum_alloc]))
    # counts[k] = number of cases assigned to bin k

    # Sort cases by raw predicted_nps ascending
...
def generate_all_arrivals(d_end: int,
                          ...
                          rho: float,
                          nps_intercept: float = NPS_PRED_INTERCEPT) -> List[Case]:
    cases: List[Case] = []
    z = 0.0
    case_id = 1
...
def queue_management(queue: List[Case], discipline: str,
    ...
    """Sort the visible queue by the active discipline. Study 3d adds NPS_BINNED."""
    ...
    if discipline == "FCFS":
        ...
        # Study 3d: priority = |binned_nps − 7.5|, random tie-break within class
        ordered = sorted(
            visible,
...
def simulate_timeline(discipline: str, n_agents: int, sla_hours: Optional[float],
    ...
    """
    Run one Study 3d simulation.

    Args:
        discipline: FCFS, SRTF, LRTF, NPS, or NPS_BINNED
        target_multinomial: Length-11 probabilities. Required for NPS_BINNED.
            Defaults to EMPIRICAL_NPS_MULTINOMIAL when None.
    """
    if seeds is None:
    ...
    # Apply rank-binning if NPS_BINNED is in play
...
def derive_seeds(replication: int, rho_idx: int) -> Dict[str, int]:
    """Same paired seeding as Study 3/3b/3c."""
...
if __name__ == "__main__":
    ...
    if EMPIRICAL_NPS_MULTINOMIAL:
        print(f"  bins: {list(range(11))}")
        print(f"  props: {[round(p, 4) for p in EMPIRICAL_NPS_MULTINOMIAL]}")
        print(f"  sum: {sum(EMPIRICAL_NPS_MULTINOMIAL):.6f}")
    print()

    for disc in ["FCFS", "LRTF", "SRTF", "NPS", "NPS_BINNED"]:
        print(f"--- discipline = {disc}, 6 agents, ρ = 1.0, hard, 60 days, rep = 1 ---")
        ...
        print(f"  closed: {result['total_cases_closed']:>4}/{result['total_cases_arrived']}")
        print(f"  avg individual NPS: {result['avg_individual_nps']:.3f}")
        print(f"  organisation NPS:   {result['organisation_nps']:+.2f}")
        print(f"  detr/pass/prom:     {result['percent_detractors']:.3f} / "
              f"{result['percent_passives']:.3f} / {result['percent_promoters']:.3f}")
        print()
